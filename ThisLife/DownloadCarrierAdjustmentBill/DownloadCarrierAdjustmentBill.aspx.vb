...
Partial Class ThisLife_DownloadCarrierAdjustmentBill_DownloadCarrierAdjustmentBill
    ...
    Private Sub timerGo_Tick(sender As Object, e As EventArgs) Handles timerGo.Tick
        ...
        Try
            ...
            strSQLSHandel = "select dtGCD.[Cell]  as 小区名 ,dtGCD.[TRX]   as 现网载波数  ,dtGCD.[NumberOfFN]  as 现网频点数 ,dtCB.[编号] ,dtCB.[NAM编码] ,dtCB.[提单单号] ,dtCB.[CELL_ID] ,dtCB.[基站名] ,dtCB.[BSC] ,dtCB.[频段] ,dtCB.[设备类型]  ,dtCB.[合路器类型]  ,dtCB.[原网载波数]  ,dtCB.[调整载波数] ,dtCB.[调整后载波数] ,dtCB.[调整合路器数（原始）] ,dtCB.[调整合路器数/DUG数] ,dtCB.[调整机架数/子框数] ,dtCB.[提单日期] ,dtCB.[备注] ,dtCB.[提单时调整类型] ,dtCB.[载波调整细类] ,dtCB.[6000的v1/v2版本]  ,dtCB.[调整类型备注(6000系列软调/硬调、换型类型）] ,dtCB.[所属区域] ,dtCB.[涉及减EDGE信道时可将EDGE信道数减容到] ,dtCB.[是否需考虑室分接入情况（是/否）] ,dtCB.[是否是(是/否/部分是)] ,dtCB.[操作内容] ,dtCB.[实际调整类型（载波调整/加向/6601远端机扩容/换型）] ,dtCB.[操作人] ,dtCB.[实施时间] ,dtNAMS.[工单号] ,dtNAMS.[工单主题] ,dtNAMS.[工单类型]  ,dtNAMS.[工单分类] ,dtNAMS.[工单状态] ,dtNAMS.[操作类型] ,dtNAMS.[区域] ,dtNAMS.[代维公司]  ,dtNAMS.[实施人] ,dtNAMS.[实施人电话] ,dtNAMS.[基站名]  ,dtNAMS.[小区名] ,dtNAMS.[派单时间] ,dtNAMS.[实施结果] ,dtNAMS.[实施完成时间] ,dtNAMS.[工单闭环时间] ,dtNAMS.[当前待办步骤]  ,dtNAMS.[当前待办人] ,dtNAMS.[无法实施归属] ,dtNAMS.[无法实施原因] ,dtNAMS.[物资来源] ,dtNAMS.[现网调整工单号] FROM ([SanShi_BaseSationDetails].[dbo].[dt_GSM_CarrierBill] dtCB left join   [SanShi_BaseSationDetails].[dbo].[dt_NAMS] dtNAMS on  dtCB.[NAM编码] = dtNAMS.[工单号] ) left join    [SanShi_BaseSationDetails].[dbo].[dt_GSMP_Cell_Daily] dtGCD  on  dtCB.[CELL_ID] = dtGCD.[Cell]       Where ( "
